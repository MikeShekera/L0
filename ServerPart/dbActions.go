...
/*
func GetJSONByOrderID(db *sql.DB, orderID string) (error, OrderJsonStructure.Order) {
	var order OrderJsonStructure.Order
	rows, err := db.Query(receiveCustomIDString, orderID)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()

	var items []OrderJsonStructure.Item
	...
	for rows.Next() {
		...
		if err != nil {
			return err, OrderJsonStructure.Order{}
		}
